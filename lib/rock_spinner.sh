...
# Rock.js Style Resilient Installation with Proper Spinners
# Matches the exact look and feel of Rock.js CLI
...
# Rock.js style loading animation with dots
resilient_show_loading() {
    local step_description="$1"
    local pid="$2"
    local delay=0.5

    # Only show animation if terminal supports it
    if [[ ! -t 1 ]]; then
        return
    fi
    ...
    while kill -0 "$pid" 2>/dev/null; do
        # Animate dots like Rock.js (cycle through ., .., ..., then reset)
        case $dot_count in
            0) dots="." ;;
            1) dots=".." ;;
            2) dots="..." ;;
        ...
        # Show the loading line (Rock.js style)
        printf "\r${ROCK_COLOR_CYAN}◇${ROCK_COLOR_RESET} %s%s" "$step_description" "$dots"
    ...
    printf "\r%*s\r" $((${#step_description} + 10)) ""
}

# Rock.js style step execution with proper loading animation
resilient_execute_step() {
    ...
    while [[ $attempt -le $RESILIENT_MAX_RETRIES ]]; do
        # Execute the step function in background
        local temp_output=$(mktemp)
        local exit_code=0

        # Start the function in background
        ("$step_function" > "$temp_output" 2>&1) &
        local bg_pid=$!

        # Show Rock.js style loading animation
        if [[ $attempt -eq 1 ]]; then
            resilient_show_loading "$step_description" "$bg_pid"
        else
            resilient_show_loading "$step_description (retry $attempt/$RESILIENT_MAX_RETRIES)" "$bg_pid"
        fi
        ...
        if [[ $exit_code -eq 0 ]]; then
            ...
            # Success - show final result (Rock.js style)
            ...
            if [[ $attempt -lt $RESILIENT_MAX_RETRIES ]]; then
                ...
                printf "${ROCK_COLOR_YELLOW}◇${ROCK_COLOR_RESET} %s ${ROCK_COLOR_DIM}(retrying...)${ROCK_COLOR_RESET}\n" "$step_description"
                sleep 1
            fi
        fi
    ...
    # All attempts failed
    printf "${ROCK_COLOR_RED}◇${ROCK_COLOR_RESET} %s ${ROCK_COLOR_DIM}(failed)${ROCK_COLOR_RESET}\n" "$step_description"
    resilient_save_step "$step_id" "failed"
    return 1
}
